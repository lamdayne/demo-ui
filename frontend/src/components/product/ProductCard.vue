<template>
  <div class="bg-white rounded-xl border border-gray-200 overflow-hidden hover:shadow-lg transition-all duration-300 flex flex-col h-full group">
    <div class="relative aspect-[4/3] bg-gray-50 overflow-hidden">
      ...
      <!-- Favorite Button -->
      <button v-if="!product.badgeLabel" class="absolute top-3 right-3 z-10 w-8 h-8 bg-white shadow-sm rounded-full flex items-center justify-center text-gray-400 hover:text-red-500 transition-colors">
        <Heart class="w-4 h-4" />
      </button>
    </div>
    ...
    <div class="p-5 flex flex-col flex-grow">
      ...
      <div class="flex flex-wrap gap-2 mb-4">
        <span v-if="product.badges.includes('Organic')" class="inline-flex items-center gap-1 bg-green-50 text-green-800 text-[10px] font-medium px-2 py-0.5 rounded-full border border-green-200">
          <Leaf class="w-3 h-3" /> Organic
        </span>
        <span v-if="product.badges.includes('Raw')" class="inline-flex items-center gap-1 bg-amber-50 text-amber-800 text-[10px] font-medium px-2 py-0.5 rounded-full border border-amber-200">
          <Leaf class="w-3 h-3" /> Raw
        </span>
        <span v-if="product.badges.includes('Lab Test')" class="inline-flex items-center gap-1 bg-blue-50 text-blue-800 text-[10px] font-medium px-2 py-0.5 rounded-full border border-blue-200">
          <FlaskConical class="w-3 h-3" /> Lab Test
        </span>
        <span v-if="product.badges.includes('VietGAP')" class="inline-flex items-center gap-1 bg-teal-50 text-teal-800 text-[10px] font-medium px-2 py-0.5 rounded-full border border-teal-200">
      ...
      <div class="mt-auto pt-2">
        ...
        <button v-if="!product.isBundle" class="w-full py-2.5 rounded-lg border border-[#1E4B35] text-[#1E4B35] font-semibold flex items-center justify-center gap-2 hover:bg-[#1E4B35] hover:text-white transition-colors">
          <ShoppingCart class="w-4 h-4" /> Add to Cart
        </button>
        <button v-else class="w-full py-2.5 rounded-lg border border-[#1E4B35] text-[#1E4B35] font-semibold flex items-center justify-center gap-2 hover:bg-[#1E4B35] hover:text-white transition-colors">
          View Bundle
        </button>
      </div>
    </div>
